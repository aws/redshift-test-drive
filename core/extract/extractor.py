...
class Extractor:
        ...
        if (
            ...
            if log_location.startswith("s3://"):
                ...
                if not match:
                    ...
                    return None
                return self.s3_extractor.get_extract_from_s3(
                    match.group(1), match.group(2), start_time, end_time
                )
    ...
    def save_logs(
        ...
        if is_s3:
            aws_service_helper.s3_put_object(
                connections_string, bucket_name, output_prefix + "/connections.json"
            )
        else:
        ...
        if is_s3:
            aws_service_helper.s3_put_object(
                replacements_string,
                bucket_name,
                output_prefix + "/copy_replacements.csv",
            )
        else:
        ...
        for filename, queries in tqdm(
            ...
            for idx, query in enumerate(queries):
                ...
                if "copy " in query.text.lower() and "from 's3:" in query.text.lower():
                    bucket = re.search(r"from 's3:\/\/[^']*", query.text, re.IGNORECASE).group()[
                        6:
                    ]
    ...
    def get_parameters_for_log_extraction(self):
        ...
        if self.config.get("log_location"):
            ...
            if "redshift-serverless" not in source_cluster_endpoint:
                result = aws_service_helper.redshift_describe_logging_status(
                ...
                if not result["LoggingEnabled"]:
                    ...
                    log_location = "s3://{}/{}".format(
                        result["BucketName"], result.get("S3KeyPrefix", "")
                    )
                logger.debug(f"Log location: {log_location}")
            # if serverless return None since the logs will be in Cloudwatch
            else:
                logger.info(
                    f"Found a redshift-serverless workload with endpoint {source_cluster_endpoint}"
                )
                log_location = "cloudwatch"
                return (extraction_name, start_time, end_time, log_location)
        else:
